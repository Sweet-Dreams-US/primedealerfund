...
export default function AdminDashboard() {
  ...
  // Identify the logged-in admin so the UI can restrict Cowork accounts to
  // the Firms + Pipeline tabs. Returns the role so callers can act on it.
  ...
  useEffect(() => {
    Promise.all([fetchMe(), fetchStats(), fetchInvestors(), fetchAllInvestors(), fetchTemplates()]).then(([role]) => {
      // Cowork accounts land on Firms (their first available tab).
      if (role === "cowork") setSection("Firms");
      setLoading(false);
    });
  }, [fetchMe, fetchStats, fetchInvestors, fetchAllInvestors, fetchTemplates]);
...
<a href="mailto:Ralph@PrimeDealerFund.com" style="color:#0563C1;text-decoration:underline;">Ralph@PrimeDealerFund.com</a>
  ...
  // Cowork accounts only see Firms + Pipeline; full admins see everything.
  const visibleSections: readonly Section[] =
    me?.role === "cowork" ? (["Firms", "Pipeline"] as Section[]) : SECTIONS;
        ...
        <div className="max-w-[1800px] mx-auto px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 border border-slate-200 px-1.5 py-0.5 rounded">{me?.role === "cowork" ? "Cowork" : "Admin"}</span>
            </div>
